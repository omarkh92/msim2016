#ifndef _PREDICTOR2_H_
#define _PREDICTOR2_H_
...
#define PRINTSIZE
#include <vector>
// use to monitor the iteration number
...
//compute the prediction
  bool predict_brcond (uint64_t PC)
  {
...
//Choser uses TAGE confidence and |LSUM|
  ...
  void HistoryUpdate (uint64_t PC, md_opcode op, bool taken,
		      ...
		      folded_history * J)
  {
    int brtype = 0;
    int i;
    bool OPTYPE_RET_UNCOND= ((MD_OP_FLAGS(op) & ( RETN )) ==( RETN ));
    bool OPTYPE_RET_COND= ((MD_OP_FLAGS(op) & ( RETN )) ==( RETN ));
    bool OPTYPE_CALL_INDIRECT_COND= ((MD_OP_FLAGS(op) & ( F_COND|F_CALL )) ==( F_COND|F_CALL ));
    bool OPTYPE_CALL_INDIRECT_UNCOND= ((MD_OP_FLAGS(op) & ( F_UNCOND|F_CALL )) ==( F_UNCOND|F_CALL ));
    bool OPTYPE_JMP_INDIRECT_UNCOND= ((MD_OP_FLAGS(op) & ( F_UNCOND|F_INDIRJMP )) ==( F_UNCOND|F_INDIRJMP ));
    bool OPTYPE_JMP_INDIRECT_COND= ((MD_OP_FLAGS(op) & ( F_COND|F_INDIRJMP )) ==( F_COND|F_INDIRJMP ));

    bool OPTYPE_JMP_DIRECT_COND= ((MD_OP_FLAGS(op) & ( F_COND|F_DIRJMP )) ==( F_COND|F_DIRJMP ));
    bool OPTYPE_CALL_DIRECT_COND= ((MD_OP_FLAGS(op) & ( F_COND|F_CALL)) ==( F_COND|F_CALL ));
    bool OPTYPE_JMP_DIRECT_UNCOND= ((MD_OP_FLAGS(op) & ( F_UNCOND|F_DIRJMP )) ==( F_UNCOND|F_DIRJMP ));
    bool OPTYPE_CALL_DIRECT_UNCOND= ((MD_OP_FLAGS(op) & ( F_UNCOND|F_CALL )) ==( F_UNCOND|F_CALL ));
    if (OPTYPE_RET_UNCOND||  OPTYPE_JMP_INDIRECT_UNCOND|| OPTYPE_JMP_INDIRECT_COND|| OPTYPE_CALL_INDIRECT_UNCOND|| OPTYPE_CALL_INDIRECT_COND||OPTYPE_RET_COND)
    {
    	brtype=1;
    }
    if (OPTYPE_JMP_DIRECT_COND|| OPTYPE_CALL_DIRECT_COND|| OPTYPE_JMP_DIRECT_UNCOND|| OPTYPE_CALL_DIRECT_UNCOND)
    {
    	brtype=2;
    }

    if (OPTYPE_JMP_DIRECT_COND || OPTYPE_CALL_DIRECT_COND|| OPTYPE_JMP_INDIRECT_COND|| OPTYPE_CALL_INDIRECT_COND|| OPTYPE_RET_COND)
    	brtype+=1;
...
// PREDICTOR UPDATE
  ...
  void update_brcond (uint64_t PC, bool resolveDir,	bool predDir, uint64_t branchTarget,md_opcode op_type)
  {
...
//END TAGE UPDATE
    ...
    HistoryUpdate (PC, op_type, resolveDir, branchTarget,
...
#define GINDEX (((long long) PC) ^ bhist ^ (bhist >> (8 - i)) ^ (bhist >> (16 - 2 * i)) ^ (bhist >> (24 - 3 * i)) ^ (bhist >> (32 - 3 * i)) ^ (bhist >> (40 - 4 * i))) & ((1 << (logs - (i >= (NBR - 2)))) - 1)
  ...
  void TrackOtherInst (uint64_t PC, md_opcode op_type, bool taken,
		       uint64_t branchTarget)
  {


    HistoryUpdate (PC, op_type, taken, branchTarget, phist,
