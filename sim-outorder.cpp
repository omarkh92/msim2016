...
public:
	...
	smt_qos_state state;
	void update(bool hit, bool taken) {
	...
	void print_stats(unsigned int c) {
		...
		cout << "BP_TAKEN[" << c << "]: " << btaken << " out of  " << total_br << ", pct " << (double)(100.0*(double)btaken/(double)total_br) << endl;
...
commit(int context_id)
{
	...
	while (contexts[context_id].ROB_num > 0 && committed < commit_width)
	{
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			...
			smtq[rs->context_id].update(correct_pred, rs->next_PC != (rs->PC +
					sizeof(md_inst_t)));
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			if (!brupdate_cond || (MD_OP_FLAGS(rs->op) & F_COND)) {
				if (!brupdate && alg1) {
					...
					bool br_taken = contexts[rs->context_id].pred->mypred->predict_brcond(rs->PC);
					contexts[rs->context_id].pred->mypred->update_brcond(rs->PC, rs->next_PC != (rs->PC + sizeof(md_inst_t)), br_taken, rs->next_PC,rs->op);
				}
			}
		}
		...
		if ((MD_OP_FLAGS(op) & F_CTRL)) {
			if (!brupdate_cond || (MD_OP_FLAGS(op) & F_COND)) {
				if (brupdate && alg1) {
					...
					bool br_taken = contexts[disp_context_id].pred->mypred->predict_brcond(regs->regs_PC);
					contexts[disp_context_id].pred->mypred->update_brcond(regs->regs_PC, regs->regs_NPC != (regs->regs_PC + sizeof(md_inst_t)), br_taken, regs->regs_NPC, op);
				}
			}
		}

		if (alg1 && ((MD_OP_FLAGS(op) & F_CTRL) || MD_OP_FLAGS(op) == RETN)) {
			if (MD_OP_FLAGS(op) & F_DIRJMP || MD_OP_FLAGS(op) & F_INDIRJMP || MD_OP_FLAGS(op) == RETN || MD_OP_FLAGS(op) & F_UNCOND) {
				contexts[disp_context_id].pred->mypred->TrackOtherInst (regs->regs_PC,op,regs->regs_NPC != (regs->regs_PC +
						sizeof(md_inst_t)) ,regs->regs_NPC);
			}
		}
			...
			i++)
	{
		...
		if (contexts[current_fetch_context].pred)
		{
	     ...
	     value may be 1 if bpred can only predict a direction */
			...
			//if (contexts[current_fetch_context].pred->mypred && alg1 && brhistory && ((MD_OP_FLAGS(op) & (F_CTRL)) == (F_CTRL))){
				//contexts[current_fetch_context].pred->mypred->FetchHistoryUpdate (contexts[current_fetch_context].fetch_regs_PC & 0x3ffff,MD_OP_FLAGS(op),
					//	takenFlag,contexts[current_fetch_context].fetch_pred_PC & 0x7f); //TODO:change in predictorcode for brtype DONE
			//}
		}
		else
		{
